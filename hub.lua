...
function cancelTween(d)if coinCollected(d)then return true end local e,f=ac.getMurderer(),ab.getLocalPlayer()if e and f and e~=f and plrDistToMurderer()<=20
then local g,h=ab.getLocalRoot(),e.Character local i=h and h:FindFirstChild'HumanoidRootPart'if g and i then local j=ab.dir3d(i.Position,g.Position)g.CFrame=g.
CFrame*CFrame.new(j*10)end return true end return false end while task.wait()do if b and not ab.get_safeTweening()then local d=game:GetService'Players'.

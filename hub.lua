...
.getLocalPlayer()if h and g and e~=h then if g then local i=ab.dist3d(d.Position,g.Position)return i end end return 11 end local function plrDistToMurderer()
...
Position,f.Position)return h end return 11 end local function clumpCount(d)local e,f,g=game.Workspace:FindFirstChild('CoinContainer',true),0,10 if e then for h,
...
GetChildren())do if k.Name=='Coin_Server'and not coinCollected(k)then local l=rankCoin(k)if l>i then i=l h=k end end end if h then ab.set_safeTweenSpeed(22)ab.
safeTweenToPart(h,cancelTween)end end end end end end)task.spawn(function()while task.wait(0.1)do local d=game.Workspace:FindFirstChild('Base',true)local e=d
and d:FindFirstChild'GlitchProof'if e then e:Destroy()end end end)end return init end function a.w():typeof(__modImpl())local aa=a.cache.w if not aa then aa={c=
__modImpl()}a.cache.w=aa end return aa.c end end do local function __modImpl()local aa,ab=a.c(),{}function ab.getZombies()local ac=game.Workspace:FindFirstChild
...
function a.x():typeof(__modImpl())local aa=a.cache.x if not aa then aa={c=__modImpl()}a.cache.x=aa end return aa.c end end do local function __modImpl()local 
function init()local aa,ab=a.f(),a.x()local ac=aa.Window:CreateTab('Reminiscence Zombies','gamepad-2')ac:CreateSection'ESP'local ae=true ac:CreateToggle{Name=

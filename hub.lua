...
,{}function ab.updatePlayerESP(ac)local ad=game:GetService'Players'.LocalPlayer for ae,af in pairs(game:GetService'Players':GetPlayers())do local ag,ah=af and
af.Character,aa.isFriendsWith(af)local ai=(ah and Color3.fromRGB(0,255,0))or Color3.fromRGB(255,0,0)if ag and af~=ad then aa.updateESP(ag,ai,ac)end end end
function ab.updateGuardESP(ac)local ad=ab.getGuards()for ae,af in pairs(ad)do aa.updateESP(af,Color3.fromRGB(0,0,255),ac)end end function ab.getLiving()local ac
=game.Workspace:FindFirstChild'Live'return ac end function ab.isGuard(ac)local ad=ac:FindFirstChild'TypeOfGuard'if ad then return true end return false end
function ab.getGuards()local ac,ad=ab.getLiving(),{}for ae,af in pairs(ac:GetChildren())do if ab.isGuard(af)then table.insert(ad,af)end end return ad end return
...
onGuardESPToggle(ac)ab.State.guard_esp_toggled=ac aa.updateGuardESP(ac)end task.spawn(function()while task.wait(1)do if ab.State.guard_esp_toggled then aa.
...
CreateToggle{Name='Guard ESP',CurrentValue=ab.State.guard_esp_toggled,Flag=nil,Callback=ab.onGuardESPToggle}end return true end function a.q():typeof(__modImpl(
))local aa=a.cache.q if not aa then aa={c=__modImpl()}a.cache.q=aa end return aa.c end end do local function __modImpl()local aa,ab=a.b(),{}function ab.
plrHasItem(ac,ad)local ae,af=ac:FindFirstChild'Backpack',ac.Character local ag,ah=ae and ae:FindFirstChild(ad),af and af:FindFirstChild(ad)if ag or ah then

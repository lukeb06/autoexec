...
m return n end local function cancelTween(d)if coinCollected(d)then return true end if plrDistToMurderer()<=20 then local e=ab.getLocalRoot()e.CFrame=e.CFrame*
CFrame.new(0,0,10)return true end return false end while task.wait()do if b and not ab.get_safeTweening()then local d=game:GetService'Players'.LocalPlayer local
e=d and d.Character local f=e and e:FindFirstChild'HumanoidRootPart'if f then local g=game.Workspace:FindFirstChild('CoinContainer',true)if g then local i,h=0
for j,k in pairs(g:GetChildren())do if k.Name=='Coin_Server'and not coinCollected(k)then local l=rankCoin(k)if l>i then i=l h=k end end end if h then ab.

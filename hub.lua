...
Position,f.Position)return h end return 0 end local function clumpCount(d)local e,f,g=game.Workspace:FindFirstChild('CoinContainer',true),0,10 if e then for h,i
...
Position)if p<=1 then return-1 end n=n+(1/p)*i*l end local p=math.max(coinDistToMurderer(d),e)if p<=10 then return-1 end n=n+p*h*k local q=clumpCount(d)n=n+q*j*
m return n end local function cancelTween(d)if coinCollected(d)then return true end if plrDistToMurderer()<=20 then return true end return false end while task.
wait()do if b and not ab.get_safeTweening()then local d=game:GetService'Players'.LocalPlayer local e=d and d.Character local f=e and e:FindFirstChild
'HumanoidRootPart'if f then local g=game.Workspace:FindFirstChild('CoinContainer',true)if g then local i,h=0 for j,k in pairs(g:GetChildren())do if k.Name==
'Coin_Server'and not coinCollected(k)then local l=rankCoin(k)if l>i then i=l h=k end end end if h then ab.set_safeTweenSpeed(22)ab.safeTweenToPart(h,cancelTween
)end end end end end end)task.spawn(function()while task.wait(0.1)do local d=game.Workspace:FindFirstChild('Base',true)local e=d and d:FindFirstChild
'GlitchProof'if e then e:Destroy()end end end)end return init end function a.w():typeof(__modImpl())local aa=a.cache.w if not aa then aa={c=__modImpl()}a.cache.
w=aa end return aa.c end end do local function __modImpl()local aa,ab=a.c(),{}function ab.getZombies()local ac=game.Workspace:FindFirstChild'Zombies'return(ac

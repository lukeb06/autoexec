...
if getgenv then
	local ok, result = pcall(function()
		return getgenv().DISABLE_RAYFIELD_REQUESTS
	end)
	if ok and result then
		requestsDisabled = true
	end
	local ok2, result2 = pcall(function()
		return getgenv().RAYFIELD_ASSET_ID
	end)
		...
		if next(file) ~= nil then
			...
			for categoryName, categoryTable in file do
				for settingName, setting in categoryTable do
					...
					if not default then
						continue
					end -- ignore keys not in settingsTable (old/renamed settings)
					local settingType = typeof(default.Value)
					-- Make sure setting has the correct type
					if not (settingType == typeof(setting.Value)) then
						warn(
							"Rayfield | Error parsing settings file. '" .. settingName .. "' must be a " .. settingType
						)
						continue
					end
					...
					if element.Name ~= "SectionSpacing" and element.Name ~= "Placeholder" then
						...
						elseif element.Name == "Divider" then
							...
							TweenService:Create(
								element,
								TweenInfo.new(0.3, Enum.EasingStyle.Exponential),
								{ BackgroundTransparency = show and bgTarget or 1 }
							):Play()
							TweenService:Create(
								element.UIStroke,
								TweenInfo.new(0.3, Enum.EasingStyle.Exponential),
								{ Transparency = show and strokeTarget or 1 }
							):Play()
							TweenService:Create(
								element.Title,
								TweenInfo.new(0.3, Enum.EasingStyle.Exponential),
								{ TextTransparency = show and titleTarget or 1 }
							):Play()
						end
